...
#################################################
spark = SparkSession.builder.appName('ML1_wf_on_NYT_5rounds_3x3_sim1_and_sim3_to_sim2_round5').config(conf=conf).getOrCreate()
...
df_original = spark.read.parquet("hdfs://192.168.0.182:9000/input/Data_NYT_clean_SPARK_START_ML2_features_round4.parquet").repartition(50)
...
# load single verdicts from round 4
verdict_r4 = spark.read.parquet("hdfs://192.168.0.182:9000/input/NYT_round4_single_verdicts.parquet").repartition(50)

# verdict to integer
verdict_r4 = verdict_r4.withColumn('verdict_r4', verdict_r4.verdict_r4.cast(IntegerType()))

# check data 
print("rows in df = ", verdict_r4.count())
print(verdict_r4.printSchema())
print(verdict_r4.show())

# merge verdict to df_original
df_original = df_original.join(verdict_r4, on='doc_id', how='left_outer')

# check data 
print("rows in df = ", df_original.count())
print(df_original.printSchema())
print(df_original.show())

# create new majortopic column
df_original = df_original.withColumn("new_mtc", coalesce(df_original.verdict_r4, df_original.majortopic))
df_original = df_original.withColumnRenamed('majortopic', 'mtc_r4')
df_original = df_original.withColumnRenamed('new_mtc', 'majortopic')
...
df_original.write.parquet("hdfs://192.168.0.182:9000/input/Data_NYT_clean_SPARK_START_ML2_features_round5.parquet", mode="overwrite")
...
for h in range(3):
    ...
    df_original = spark.read.parquet("hdfs://192.168.0.182:9000/input/Data_NYT_clean_SPARK_START_ML2_features_round5.parquet").repartition(50)
    ...
    for i in majortopic_codes:
        ...
        df_train = df_original.where((col('sim') != 2) | (col('verdict_r1').isNotNull()) | (col('verdict_r2').isNotNull()) | (col('verdict_r3').isNotNull()) | (col('verdict_r4').isNotNull()))
        df_test = df_original.where((col('sim') == 2) & (col('verdict_r1').isNull()) & (col('verdict_r2').isNull()) & (col('verdict_r3').isNull()) & (col('verdict_r4').isNull()))
    ...
    df_numbers.to_csv("NYT_round5_sample{h}_sample_numbers.csv".format(h=h+1), index=False)
    ...
    for j in range(3):
        ...
        df.to_csv("NYT_round5_sample{h}_svm{j}.csv".format(h=h+1,j=j), index=False)
...
for i in range(3):
    for j in range(3):
        df = pd.read_csv("NYT_round5_sample{i}_svm{j}.csv".format(i=i+1, j=j))
...
df_results["verdict_r5"] = df_results.iloc[:,1:].sum(1)

df_results = df_results[["doc_id", "verdict_r5"]]

# write to csv
df_results.to_csv("NYT_round5_single_verdicts.csv", index=False)
...
df_results.write.parquet("hdfs://192.168.0.182:9000/input/NYT_round5_single_verdicts.parquet", mode="overwrite")
