...
#################################################
spark = SparkSession.builder.appName('ML1_wf_on_NYT_5rounds_3x3_sim1_and_sim3_to_sim2_round3').config(conf=conf).getOrCreate()
...
df_original = spark.read.parquet("hdfs://192.168.0.182:9000/input/Data_NYT_clean_SPARK_START_ML2_features_round2.parquet").repartition(50)
...
# load single verdicts from round 2
verdict_r2 = spark.read.parquet("hdfs://192.168.0.182:9000/input/NYT_round2_single_verdicts.parquet").repartition(50)

# verdict to integer
verdict_r2 = verdict_r2.withColumn('verdict_r2', verdict_r2.verdict_r2.cast(IntegerType()))

# check data 
print("rows in df = ", verdict_r2.count())
print(verdict_r2.printSchema())
print(verdict_r2.show())

# merge verdict to df_original
df_original = df_original.join(verdict_r2, on='doc_id', how='left_outer')
...
df_original = df_original.withColumn("new_mtc", coalesce(df_original.verdict_r2, df_original.majortopic))
df_original = df_original.withColumnRenamed('majortopic', 'mtc_r2')
...
df_original.write.parquet("hdfs://192.168.0.182:9000/input/Data_NYT_clean_SPARK_START_ML2_features_round3.parquet", mode="overwrite")
...
for h in range(3):
    ...
    for i in majortopic_codes:
        ...
        df_train = df_original.where((col('sim') != 2) | (col('verdict_r1').isNotNull()) | (col('verdict_r2').isNotNull()))
        df_test = df_original.where((col('sim') == 2) & (col('verdict_r1').isNull()) & (col('verdict_r2').isNull()))
    ...
    print("ALL SVM DONE round3_{h}".format(h=h+1))

    df_numbers.to_csv("NYT_round3_sample{h}_sample_numbers.csv".format(h=h+1), index=False)
    ...
    for j in range(3):
        ...
        df.to_csv("NYT_round3_sample{h}_svm{j}.csv".format(h=h+1,j=j), index=False)
...
for i in range(3):
    for j in range(3):
        df = pd.read_csv("NYT_round3_sample{i}_svm{j}.csv".format(i=i+1, j=j))
...
df_results["verdict_r3"] = df_results.iloc[:,1:].sum(1)

df_results = df_results[["doc_id", "verdict_r3"]]

# write to csv
df_results.to_csv("NYT_round3_single_verdicts.csv", index=False)
...
df_results.write.parquet("hdfs://192.168.0.182:9000/input/NYT_round3_single_verdicts.parquet", mode="overwrite")
