...
#################################################
spark = SparkSession.builder.appName('ML1_wf_on_NYT_5rounds_3x3_sim1_and_sim3_to_sim2_round2').config(conf=conf).getOrCreate()
...
for h in range(3):
    ...
    for j in range(3):
        ...
        df.to_csv("NYT_round2_sample{h}_svm{j}.csv".format(h=h+1,j=j), index=False)
...
for i in range(3):
    for j in range(3):
        df = pd.read_csv("NYT_round2_sample{i}_svm{j}.csv".format(i=i+1, j=j))
