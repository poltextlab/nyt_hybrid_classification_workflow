...
#################################################
spark = SparkSession.builder.appName('ML1_wf_on_NYT_5rounds_3x3_sim1_and_sim3_to_sim2_round1').config(conf=conf).getOrCreate()
...
for i in range(3):
    for j in range(3):
        df = pd.read_csv("NYT_round1_sample{i}_svm{j}.csv".format(i=i+1, j=j))
...
df_results.write.parquet("hdfs://192.168.0.182:9000/input/NYT_round1_single_verdicts.parquet", mode="overwrite")
