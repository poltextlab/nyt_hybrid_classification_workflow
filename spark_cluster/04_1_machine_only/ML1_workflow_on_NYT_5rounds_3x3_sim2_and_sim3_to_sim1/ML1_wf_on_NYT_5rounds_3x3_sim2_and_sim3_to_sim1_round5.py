...
#################################################
spark = SparkSession.builder.appName('ML1_wf_on_NYT_5rounds_3x3_sim2_and_sim3_to_sim1_round5').config(conf=conf).getOrCreate()
...
for h in range(3):
    ...
    for i in majortopic_codes:
        ...
        df_train = df_original.where((col('sim') != 1) | (col('verdict_r1').isNotNull()) | (col('verdict_r2').isNotNull()) | (col('verdict_r3').isNotNull()) | (col('verdict_r4').isNotNull()))
        df_test = df_original.where((col('sim') == 1) & (col('verdict_r1').isNull()) & (col('verdict_r2').isNull()) & (col('verdict_r3').isNull()) & (col('verdict_r4').isNull()))
