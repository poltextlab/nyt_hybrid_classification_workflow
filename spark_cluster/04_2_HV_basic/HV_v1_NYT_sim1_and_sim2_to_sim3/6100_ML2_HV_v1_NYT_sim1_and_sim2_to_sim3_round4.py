...
#################################################
spark = SparkSession.builder.appName('ML2_HV_v1_NYT_sim1_and_sim2_to_sim3_round4').config(conf=conf).getOrCreate()
...
for h in range(3):
    ...
    for i in majortopic_codes:
        ...
        df_train = df_original.where(col('train_r4') == 1)
        df_test = df_original.where(col('train_r4') == 0)
    ...
    df_numbers.to_csv("ML2_HV_v1_NYT_round4_sample{h}_sample_numbers.csv".format(h=h+1), index=False)
    ...
    for j in range(3):
        ...
        df.to_csv("ML2_HV_v1_NYT_round4_sample{h}_svm{j}.csv".format(h=h+1,j=j), index=False)
...
for i in range(3):
    for j in range(3):
        df = pd.read_csv("ML2_HV_v1_NYT_round4_sample{i}_svm{j}.csv".format(i=i+1, j=j))
...
df_original.write.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v1_NYT_r4_train_and_remaining_NOTclassified.parquet", mode="overwrite")
df_results.write.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v1_NYT_r4_classified.parquet", mode="overwrite")
...
df_original.to_csv("ML2_HV_v1_NYT_r4_train_and_remaining_NOTclassified.csv", index=False)
df_results.to_csv("ML2_HV_v1_NYT_r4_classified.csv", index=False)
