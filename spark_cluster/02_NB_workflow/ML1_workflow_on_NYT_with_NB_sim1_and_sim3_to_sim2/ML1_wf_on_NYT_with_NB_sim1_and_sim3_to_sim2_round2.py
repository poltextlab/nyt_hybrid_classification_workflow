...
#################################################
spark = SparkSession.builder.appName('ML1_wf_on_NYT_with_NB_sim1_and_sim3_to_sim2_round2').config(conf=conf).getOrCreate()
...
for h in range(7):
    ...
    for i in majortopic_codes:
        ...
        df_train = df_original.where((col('sim') != 2) | (col('verdict_r1').isNotNull()))
        df_test = df_original.where((col('sim') == 2) & (col('verdict_r1').isNull()))
...
df_results.write.parquet("hdfs://192.168.0.182:9000/input/NYT_round2_single_verdicts.parquet", mode="overwrite")
