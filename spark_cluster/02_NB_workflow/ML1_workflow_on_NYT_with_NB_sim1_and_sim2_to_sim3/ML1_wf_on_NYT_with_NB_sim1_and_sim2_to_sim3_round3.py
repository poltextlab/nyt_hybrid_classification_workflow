...
#################################################
spark = SparkSession.builder.appName('ML1_wf_on_NYT_with_NB_sim1_and_sim2_to_sim3_round3').config(conf=conf).getOrCreate()
...
for h in range(7):
    ...
    print("ALL NB DONE round3_{h}".format(h=h+1))
    ...
    for j in range(7):
        ...
        df.to_csv("NYT_round3_sample{h}_nb{j}.csv".format(h=h+1,j=j), index=False)
...
for i in range(7):
    for j in range(7):
        df = pd.read_csv("NYT_round3_sample{i}_nb{j}.csv".format(i=i+1, j=j))
...
df_results.write.parquet("hdfs://192.168.0.182:9000/input/NYT_round3_single_verdicts_NB.parquet", mode="overwrite")
