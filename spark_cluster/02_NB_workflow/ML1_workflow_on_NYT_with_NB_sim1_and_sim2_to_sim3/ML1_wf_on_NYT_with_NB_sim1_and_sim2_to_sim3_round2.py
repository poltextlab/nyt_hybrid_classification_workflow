...
#################################################
spark = SparkSession.builder.appName('ML1_wf_on_NYT_with_NB_sim1_and_sim2_to_sim3_round2').config(conf=conf).getOrCreate()
...
# load single verdicts from round 1
verdict_r1 = spark.read.parquet("hdfs://192.168.0.182:9000/input/NYT_round1_single_verdicts.parquet").repartition(50)

# verdict to integer
verdict_r1 = verdict_r1.withColumn('verdict_r1', verdict_r1.verdict_r1.cast(IntegerType()))

# check data 
print("rows in df = ", verdict_r1.count())
print(verdict_r1.printSchema())
print(verdict_r1.show())

# merge verdict to df_original
df_original = df_original.join(verdict_r1, on='doc_id', how='left_outer')
...
df_original = df_original.withColumn("new_mtc", coalesce(df_original.verdict_r1, df_original.majortopic))
df_original = df_original.withColumnRenamed('majortopic', 'mtc_r1')
...
df_original.write.parquet("hdfs://192.168.0.182:9000/input/Data_NYT_clean_SPARK_START_ML2_features_round2.parquet", mode="overwrite")
...
for h in range(7):
    ...
    for i in majortopic_codes:
        ...
        df_train = df_original.where((col('sim') != 3) | (col('verdict_r1').isNotNull()))
        df_test = df_original.where((col('sim') == 3) & (col('verdict_r1').isNull()))
    ...
    print("ALL NB DONE round2_{h}".format(h=h+1))

    df_numbers.to_csv("NYT_round2_sample{h}_sample_numbers.csv".format(h=h+1), index=False)
    ...
    for j in range(7):
        ...
        df.to_csv("NYT_round2_sample{h}_nb{j}.csv".format(h=h+1,j=j), index=False)
...
for i in range(7):
    for j in range(7):
        df = pd.read_csv("NYT_round2_sample{i}_nb{j}.csv".format(i=i+1, j=j))
...
df_results["verdict_r2"] = df_results.iloc[:,1:].sum(1)

df_results = df_results[["doc_id", "verdict_r2"]]

# write to csv
df_results.to_csv("NYT_round2_single_verdicts.csv", index=False)
...
df_results.write.parquet("hdfs://192.168.0.182:9000/input/NYT_round2_single_verdicts_NB.parquet", mode="overwrite")
