...
#################################################
spark = SparkSession.builder.appName('ML1_wf_on_NYT_with_NB_sim2_and_sim3_to_sim1_round2').config(conf=conf).getOrCreate()
...
for h in range(7):
    ...
    for i in majortopic_codes:
        ...
        df_train = df_original.where((col('sim') != 1) | (col('verdict_r1').isNotNull()))
        df_test = df_original.where((col('sim') == 1) & (col('verdict_r1').isNull()))
