...
#################################################
spark = SparkSession.builder.appName('ML2_HV_v2_savNegInf_NYT_sim1_and_sim2_to_sim3_round2').config(conf=conf).getOrCreate()
...
for h in range(3):
    ...
    df_original = spark.read.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v2_savNegInf_NYT_round2_start.parquet").repartition(50)
    ...
    df_numbers.to_csv("ML2_HV_v2_savNegInf_NYT_round2_sample{h}_sample_numbers.csv".format(h=h+1), index=False)
    ...
    for j in range(3):
        ...
        df.to_csv("ML2_HV_v2_savNegInf_NYT_round2_sample{h}_svm{j}.csv".format(h=h+1,j=j), index=False)
...
for i in range(3):
    for j in range(3):
        df = pd.read_csv("ML2_HV_v2_savNegInf_NYT_round2_sample{i}_svm{j}.csv".format(i=i+1, j=j))
...
df_original.write.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v2_savNegInf_NYT_r2_train_and_remaining_NOTclassified.parquet", mode="overwrite")
df_results.write.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v2_savNegInf_NYT_r2_classified.parquet", mode="overwrite")
...
df_original.to_csv("ML2_HV_v2_savNegInf_NYT_r2_train_and_remaining_NOTclassified.csv", index=False)
df_results.to_csv("ML2_HV_v2_savNegInf_NYT_r2_classified.csv", index=False)
