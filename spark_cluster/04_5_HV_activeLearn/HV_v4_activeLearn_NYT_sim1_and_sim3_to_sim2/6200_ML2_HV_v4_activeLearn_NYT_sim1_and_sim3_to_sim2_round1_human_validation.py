...
#################################################
spark = SparkSession.builder.appName('ML2_HV_v4_activeLearn_NYT_sim1_and_sim3_to_sim2_round1_human_validation').config(conf=conf).getOrCreate()
...
df_results = spark.read.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_r1_classified.parquet").repartition(50)
...
df_numbers.to_csv("ML2_HV_v4_activeLearn_NYT_human_validation_numbers_r1.csv", index=True)
...
df_original = spark.read.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_r1_train_and_remaining_NOTclassified.parquet").repartition(50)
...
df_activeLearn = spark.read.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_r1_activeLearn.parquet").repartition(50)
# create column for known negatives in df_activeLearn
df_activeLearn = df_activeLearn.withColumn('train_r2_neg', lit(0))
# we need to create a new majortopic column, because we are now adding back in elements with
# potentially new labels
df_activeLearn = df_activeLearn.withColumnRenamed('majortopic', 'mtc_original')
df_activeLearn = df_activeLearn.withColumn('majortopic', df_activeLearn['mtc_original'])
# create the new train id column
df_activeLearn = df_activeLearn.withColumn("train_r2", lit(1))
...
df_original.write.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_round2_start.parquet", mode="overwrite")
...
df_original.to_csv("ML2_HV_v4_activeLearn_NYT_round2_starting_table.csv", index=False)
