...
#################################################
spark = SparkSession.builder.appName('ML2_HV_v4_activeLearn_NYT_sim1_and_sim3_to_sim2_round2_human_validation').config(conf=conf).getOrCreate()
...
#################################################
# read result data from round 2
#################################################

df_results = spark.read.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_r2_classified.parquet").repartition(50)
...
# where precision is equal to or greater than 80%
# NOTE: validated wrongly classified elements will NOT be added to the results with the wrong major
# topic code, instead they will be added to the unclassified
df_replace_all = None
# where precision is less than 80%
...
for i in majortopic_codes:
    ...
    if df_numbers["precision_in_sample"].loc[i] >= 0.80:
...
# sometimes there will be no major topic code with precision => 80%
if df_replace_all == None:
...
df_numbers.to_csv("ML2_HV_v4_activeLearn_NYT_human_validation_numbers_r2.csv", index=True)
...
df_original = spark.read.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_r2_train_and_remaining_NOTclassified.parquet").repartition(50)
# create column for known negatives in df_original
df_original = df_original.withColumn('train_r3_neg', lit(0))
# we need to create a new majortopic column, because we are now adding back in elements with
# potentially new labels
df_original = df_original.withColumnRenamed('majortopic', 'mtc_after_r1')
df_original = df_original.withColumn('majortopic', df_original['mtc_after_r1'])
# finally, create the new train id column
df_original = df_original.withColumn("train_r3", when(df_original["train_r2"] == 1, 1).otherwise(0))

#################################################
# add df_activeLearn to df_original
#################################################

df_activeLearn = spark.read.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_r2_activeLearn.parquet").repartition(50)
# create column for known negatives in df_activeLearn
df_activeLearn = df_activeLearn.withColumn('train_r3_neg', lit(0))
# we need to create a new majortopic column, because we are now adding back in elements with
# potentially new labels
df_activeLearn = df_activeLearn.withColumnRenamed('majortopic', 'mtc_after_r1')
df_activeLearn = df_activeLearn.withColumn('majortopic', df_activeLearn['mtc_after_r1'])
# create the new train id column
df_activeLearn = df_activeLearn.withColumn("train_r3", lit(1))
# drop the extra columns to be able to add it back to df_original
df_activeLearn = df_activeLearn.drop('verdict')

# add df_activeLearn elements to df_original
df_original = df_original.union(df_activeLearn)
...
if df_replace_all != "empty":
    ...
    df_replace_all = df_replace_all.withColumn('train_r3_neg', lit(0))
    # we need to create a new majortopic column, because we are now adding back in elements with
    # potentially new labels
    df_replace_all = df_replace_all.withColumnRenamed('majortopic', 'mtc_after_r1')
    df_replace_all = df_replace_all.withColumn('majortopic', df_replace_all['verdict'])
    # create the new train id column
    df_replace_all = df_replace_all.withColumn("train_r3", lit(1))
    # drop the extra columns to be able to add it back to df_original
...
if df_non_sample_replace != "empty":
    ...
    df_non_sample_replace = df_non_sample_replace.withColumn('train_r3_neg', lit(0))
    # we need to create a new majortopic column, because we are now adding back in elements with
    # potentially new labels
    df_non_sample_replace = df_non_sample_replace.withColumnRenamed('majortopic', 'mtc_after_r1')
    df_non_sample_replace = df_non_sample_replace.withColumn('majortopic', df_non_sample_replace['mtc_after_r1'])
    # create the new train id column
    df_non_sample_replace = df_non_sample_replace.withColumn("train_r3", lit(0))
    # drop the extra columns to be able to add it back to df_original
...
df_correct_replace = df_correct_replace.withColumn('train_r3_neg', lit(0))
# we need to create a new majortopic column, because we are now adding back in elements with
# potentially new labels
df_correct_replace = df_correct_replace.withColumnRenamed('majortopic', 'mtc_after_r1')
df_correct_replace = df_correct_replace.withColumn('majortopic', df_correct_replace['verdict'])
# create the new train id column
df_correct_replace = df_correct_replace.withColumn("train_r3", lit(1))
# drop the extra columns to be able to add it back to df_original
...
df_wrong_replace = df_wrong_replace.withColumn('train_r3_neg', df_wrong_replace['verdict'])
# we need to create a new majortopic column, because we are now adding back in elements with
# potentially new labels
df_wrong_replace = df_wrong_replace.withColumnRenamed('majortopic', 'mtc_after_r1')
df_wrong_replace = df_wrong_replace.withColumn('majortopic', df_wrong_replace['mtc_after_r1'])
# create the new train id column
df_wrong_replace = df_wrong_replace.withColumn("train_r3", lit(0))
# drop the extra columns to be able to add it back to df_original
...
df_original.write.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_round3_start.parquet", mode="overwrite")

df_original.groupBy("train_r3").count().show(n=30)
...
df_original = spark.read.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_round3_start.parquet").repartition(50)
df_original = df_original.drop('text', 'words', 'features', 'raw_features').toPandas()
df_original.to_csv("ML2_HV_v4_activeLearn_NYT_round3_starting_table.csv", index=False)
