...
#################################################
spark = SparkSession.builder.appName('ML2_HV_v4_activeLearn_NYT_sim1_and_sim3_to_sim2_round4_human_validation').config(conf=conf).getOrCreate()
...
#################################################
# read result data from round 4
#################################################

df_results = spark.read.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_r4_classified.parquet").repartition(50)
...
df_numbers.to_csv("ML2_HV_v4_activeLearn_NYT_human_validation_numbers_r4.csv", index=True)
...
df_original = spark.read.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_r4_train_and_remaining_NOTclassified.parquet").repartition(50)
# create column for known negatives in df_original
df_original = df_original.withColumn('train_r5_neg', lit(0))
...
df_activeLearn = spark.read.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_r4_activeLearn.parquet").repartition(50)
...
if len(df_activeLearn.columns) != 1:
    ...
    df_activeLearn = df_activeLearn.withColumn('train_r5_neg', lit(0))
    # we need to create a new majortopic column, because we are now adding back in elements with
    # potentially new labels
    df_activeLearn = df_activeLearn.withColumnRenamed('majortopic', 'mtc_after_r3')
    df_activeLearn = df_activeLearn.withColumn('majortopic', df_activeLearn['mtc_after_r3'])
    # create the new train id column
    df_activeLearn = df_activeLearn.withColumn("train_r5", lit(1))
...
if df_replace_all != "empty":
    ...
    df_replace_all = df_replace_all.withColumn('train_r5_neg', lit(0))
    # we need to create a new majortopic column, because we are now adding back in elements with
    # potentially new labels
...
if df_non_sample_replace != "empty":
    ...
    df_non_sample_replace = df_non_sample_replace.withColumn('train_r5_neg', lit(0))
    # we need to create a new majortopic column, because we are now adding back in elements with
    # potentially new labels
...
df_correct_replace = df_correct_replace.withColumn('train_r5_neg', lit(0))
# we need to create a new majortopic column, because we are now adding back in elements with
# potentially new labels
...
df_wrong_replace = df_wrong_replace.withColumn('train_r5_neg', df_wrong_replace['verdict'])
# we need to create a new majortopic column, because we are now adding back in elements with
# potentially new labels
...
df_original.write.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_round5_start.parquet", mode="overwrite")
...
df_original.to_csv("ML2_HV_v4_activeLearn_NYT_round5_starting_table.csv", index=False)
