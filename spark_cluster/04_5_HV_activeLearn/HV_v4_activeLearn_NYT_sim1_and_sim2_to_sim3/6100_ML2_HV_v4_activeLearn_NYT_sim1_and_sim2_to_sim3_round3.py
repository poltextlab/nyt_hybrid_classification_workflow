...
#################################################
spark = SparkSession.builder.appName('ML2_HV_v4_activeLearn_NYT_sim1_and_sim2_to_sim3_round3').config(conf=conf).getOrCreate()
...
for h in range(3):
    ...
    for i in majortopic_codes:
        ...
        df_train = df_original.where((col('train_r3') == 1) | (col('train_r2_neg') == i) | (col('train_r3_neg') == i))
        df_test = df_original.where((col('train_r3') == 0) & (col('train_r2_neg') != i) & (col('train_r3_neg') != i))
    ...
    df_numbers.to_csv("ML2_HV_v4_activeLearn_NYT_round3_sample{h}_sample_numbers.csv".format(h=h+1), index=False)
    ...
    for j in range(3):
        ...
        df.to_csv("ML2_HV_v4_activeLearn_NYT_round3_sample{h}_svm{j}.csv".format(h=h+1,j=j), index=False)
...
for i in range(3):
    for j in range(3):
        df = pd.read_csv("ML2_HV_v4_activeLearn_NYT_round3_sample{i}_svm{j}.csv".format(i=i+1, j=j))
...
df_activeLearn.to_csv("ML2_v4_activeLearn_NYT_r3_activeLearn_raw.csv", index=False)
...
df_original.write.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_r3_train_and_remaining_NOTclassified.parquet", mode="overwrite")
df_results.write.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_r3_classified.parquet", mode="overwrite")
df_results_al.write.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_r3_activeLearn.parquet", mode="overwrite")
...
df_original.to_csv("ML2_HV_v4_activeLearn_NYT_r3_train_and_remaining_NOTclassified.csv", index=False)
df_results.to_csv("ML2_HV_v4_activeLearn_NYT_r3_classified.csv", index=False)
if len(df_al.columns) != 1:
    df_results_al.to_csv("ML2_HV_v4_activeLearn_NYT_r3_activeLearn.csv", index=False)
