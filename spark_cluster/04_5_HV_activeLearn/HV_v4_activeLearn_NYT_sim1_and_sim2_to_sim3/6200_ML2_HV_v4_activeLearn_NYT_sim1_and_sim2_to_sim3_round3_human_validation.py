...
#################################################
spark = SparkSession.builder.appName('ML2_HV_v4_activeLearn_NYT_sim1_and_sim2_to_sim3_round3_human_validation').config(conf=conf).getOrCreate()
...
df_results = spark.read.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_r3_classified.parquet").repartition(50)
...
df_numbers.to_csv("ML2_HV_v4_activeLearn_NYT_human_validation_numbers_r3.csv", index=True)
...
df_original = spark.read.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_r3_train_and_remaining_NOTclassified.parquet").repartition(50)
# create column for known negatives in df_original
df_original = df_original.withColumn('train_r4_neg', lit(0))
# we need to create a new majortopic column, because we are now adding back in elements with
# potentially new labels
df_original = df_original.withColumnRenamed('majortopic', 'mtc_after_r2')
df_original = df_original.withColumn('majortopic', df_original['mtc_after_r2'])
# finally, create the new train id column
df_original = df_original.withColumn("train_r4", when(df_original["train_r3"] == 1, 1).otherwise(0))

#################################################
# add df_activeLearn to df_original
#################################################

df_activeLearn = spark.read.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_r3_activeLearn.parquet").repartition(50)
# check whether df_activeLearn is empty or not
if len(df_activeLearn.columns) != 1:
    # create column for known negatives in df_activeLearn
    df_activeLearn = df_activeLearn.withColumn('train_r4_neg', lit(0))
    # we need to create a new majortopic column, because we are now adding back in elements with
    # potentially new labels
    df_activeLearn = df_activeLearn.withColumnRenamed('majortopic', 'mtc_after_r2')
    df_activeLearn = df_activeLearn.withColumn('majortopic', df_activeLearn['mtc_after_r2'])
    # create the new train id column
    df_activeLearn = df_activeLearn.withColumn("train_r4", lit(1))
...
if df_replace_all != "empty":
    ...
    df_replace_all = df_replace_all.withColumn('train_r4_neg', lit(0))
    # we need to create a new majortopic column, because we are now adding back in elements with
    # potentially new labels
    df_replace_all = df_replace_all.withColumnRenamed('majortopic', 'mtc_after_r2')
    df_replace_all = df_replace_all.withColumn('majortopic', df_replace_all['verdict'])
    # create the new train id column
    df_replace_all = df_replace_all.withColumn("train_r4", lit(1))
    # drop the extra columns to be able to add it back to df_original
...
if df_non_sample_replace != "empty":
    ...
    df_non_sample_replace = df_non_sample_replace.withColumn('train_r4_neg', lit(0))
    # we need to create a new majortopic column, because we are now adding back in elements with
    # potentially new labels
    df_non_sample_replace = df_non_sample_replace.withColumnRenamed('majortopic', 'mtc_after_r2')
    df_non_sample_replace = df_non_sample_replace.withColumn('majortopic', df_non_sample_replace['mtc_after_r2'])
    # create the new train id column
    df_non_sample_replace = df_non_sample_replace.withColumn("train_r4", lit(0))
    # drop the extra columns to be able to add it back to df_original
...
df_correct_replace = df_correct_replace.withColumn('train_r4_neg', lit(0))
# we need to create a new majortopic column, because we are now adding back in elements with
# potentially new labels
df_correct_replace = df_correct_replace.withColumnRenamed('majortopic', 'mtc_after_r2')
df_correct_replace = df_correct_replace.withColumn('majortopic', df_correct_replace['verdict'])
# create the new train id column
df_correct_replace = df_correct_replace.withColumn("train_r4", lit(1))
# drop the extra columns to be able to add it back to df_original
...
df_wrong_replace = df_wrong_replace.withColumn('train_r4_neg', df_wrong_replace['verdict'])
# we need to create a new majortopic column, because we are now adding back in elements with
# potentially new labels
df_wrong_replace = df_wrong_replace.withColumnRenamed('majortopic', 'mtc_after_r2')
df_wrong_replace = df_wrong_replace.withColumn('majortopic', df_wrong_replace['mtc_after_r2'])
# create the new train id column
df_wrong_replace = df_wrong_replace.withColumn("train_r4", lit(0))
# drop the extra columns to be able to add it back to df_original
...
df_original.write.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_round4_start.parquet", mode="overwrite")

df_original.groupBy("train_r4").count().show(n=30)
...
df_original = spark.read.parquet("hdfs://192.168.0.182:9000/input/ML2_HV_v4_activeLearn_NYT_round4_start.parquet").repartition(50)
df_original = df_original.drop('text', 'words', 'features', 'raw_features').toPandas()
df_original.to_csv("ML2_HV_v4_activeLearn_NYT_round4_starting_table.csv", index=False)
