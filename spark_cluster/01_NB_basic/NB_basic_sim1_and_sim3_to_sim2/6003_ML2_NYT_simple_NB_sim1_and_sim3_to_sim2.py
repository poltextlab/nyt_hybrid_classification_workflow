...
#################################################
spark = SparkSession.builder.appName('ML2_NYT_simple_NB_sim1_and_sim3_to_sim2').config(conf=conf).getOrCreate()
...
df_train = df.where(col('sim') != 2)
df_test = df.where(col('sim') == 2)
...
df.to_csv("ML2_NYT_simple_NB_sim1_and_sim3_to_sim2.csv", index=False)
